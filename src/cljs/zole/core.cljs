...
(defn play-against-bots [state]
  (connect! state (fn [s ws-chan]
                    (let [name  (str "player-" (str (random-uuid)))
                      ...
                      (go (println "!" ws-chan (>! ws-chan ["login" name]))
                          (>! ws-chan ["join" table]))))))
...
(defn handle-message [state msg]
  ...
  (cond
    ...
    (= (first msg) "end_of_game") (swap! state assoc-in [:play-page :game-type] nil)
